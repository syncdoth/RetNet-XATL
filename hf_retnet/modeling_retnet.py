...
try:
    from fla.ops.retention.parallel import parallel_retention as fused_parallel_retention
    from fla.ops.retention.recurrent_fuse import fused_recurrent_retention
    from fla.ops.retention.chunk_fuse import fused_chunk_retention
except ModuleNotFoundError:
    fused_parallel_retention = None
    fused_recurrent_retention = None
...
def split_heads(tensors, bsz, seqlen, num_heads):
    ...
    return [x.view(bsz, seqlen, num_heads, -1) for x in tensors]


def rotate_every_two(x, interleave=True):
    # NOTE: this is interleaved=True
    if interleave:
        x1 = x[..., ::2]
        x2 = x[..., 1::2]
    else:
...
def theta_shift(x, sin, cos, interleave=True):
    return (x * cos) + (rotate_every_two(x, interleave=interleave) * sin)
...
class RetNetRelPos(nn.Module):
    ...
    def __init__(self, config: RetNetConfig):
        ...
        if config.use_lm_decay:
            ...
            decay = torch.log2(1 - 2.**(-5. - torch.arange(num_heads)))
    ...
    def forward(
        ...
        if forward_impl == "recurrent":
            ...
            retention_rel_pos = ((sin.unsqueeze(0), cos.unsqueeze(0)),
                                 self.decay.view(1, -1, 1, 1).exp2())
            ...
            if self.config.retnet_stability_update:
            ...
            if self.config.retnet_stability_update:
                query_inner_decay = query_inner_decay / scale
            ...
            if retention_mask is not None:
                ...
                mask = mask * retention_mask.view(-1, 1, 1, slen).to(mask)
    ...
    def compute_decay_scale(self, slen, retention_mask=None):
        exponent = torch.arange(slen).to(self.decay)
...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(
        ...
        if config.retnet_bottleneck:
            self.down_proj = nn.Linear(self.embed_dim, config.retnet_bottleneck, bias=use_bias)
            self.g_proj = nn.Linear(self.embed_dim, config.retnet_bottleneck, bias=use_bias)
            self.out_proj = nn.Linear(config.retnet_bottleneck, self.embed_dim, bias=use_bias)
        else:
            self.down_proj = None
    ...
    def parallel_retention(self, q, k, v, decay_mask, use_cache=True):
        ...
        if fused_parallel_retention is not None:
            output = fused_parallel_retention(q, k, v)
            ...
            if self.config.retnet_stability_update:
                denom = retention.detach().abs().sum(dim=-1, keepdim=True).clamp(min=1, max=5e4)
                retention = retention / denom
    ...
    def recurrent_retention(self, q, k, v, decay, past_key_value=None, retention_mask=None):
        ...
        """
        if fused_recurrent_retention is not None and retention_mask is None:
            # TODO: this is not implemented for retention_mask, esp. when there is left padding.
            if past_key_value is not None:
                past_kv = past_key_value["prev_key_value"].transpose(2, 3)  # (b,h,v,q) -> (b,h,q,v)
            else:
                past_kv = None
            output, current_kv = fused_recurrent_retention(q, k, v, past_kv, output_final_state=True)
            output = output.transpose(1, 2)  # (b,h,1,v) -> # (b,1,h,v)
            current_kv = current_kv.transpose(2, 3)  # (b,h,q,v) -> (b,h,v,q)
            cache = {"prev_key_value": current_kv, "scale": None}
            return output, cache

        if retention_mask is not None:
            retention_mask = retention_mask.view(-1, 1, 1, 1).to(decay)
        else:
        ...
        if past_key_value is not None and "prev_key_value" in past_key_value:
            ...
            if self.config.retnet_stability_update:
                ...
                decay = prev_scale.sqrt() * decay / scale.sqrt()
            # how much to decay prev_kv
            decay_amount = torch.where(retention_mask == 0, 1, decay)
        ...
        elif self.config.retnet_stability_update:
        ...
        output = torch.sum(q * self.scaling * current_kv, dim=3).unsqueeze(1) # (b, 1, h, d_v)
    ...
    def chunkwise_retention(self, q, k, v, decay_mask):
        ...
        """
        # TODO: output is correct, but the cache states are incorrect.
        if fused_chunk_retention is not None:
            output, current_kv = fused_chunk_retention(q, k, v, output_final_state=True)
            output = output.transpose(1, 2)
            current_kv = current_kv.transpose(2, 3)
            cache = {"prev_key_value": current_kv, "scale": None}
            return output, cache
        (
            decay_mask,
        ...
        if self.config.retnet_stability_update:
            inner_scale = qk_mat.detach().abs().sum(dim=-1, keepdim=True).clamp(min=1, max=5e4)
        ...
        for i in range(num_chunks):
            if self.config.retnet_stability_update:
                kv_state = kv_state / kv_scale
                cross_scale.append(kv_scale)
            kv_recurrent.append(kv_state)
            kv_state = kv_state * cross_decay + kv[:, i]
            if self.config.retnet_stability_update:
                state_sum = kv_state.detach().abs().sum(dim=-2, keepdim=True)
                kv_scale = state_sum.max(dim=-1, keepdim=True).values.clamp(min=1, max=5e4)
        ...
        if self.config.retnet_stability_update:
            ...
            inner_output = inner_output / align_inner_scale
            cross_output = cross_output / align_cross_scale
    ...
    def forward(
        ...
        if apply_rotary_emb_func is not None:
            # (B, T, n_h, h_dim)
            qr = apply_rotary_emb_func(q, cos, sin, True, True)
            kr = apply_rotary_emb_func(k, cos, sin, True, True)
        else:
            n_elem = int(self.config.decoder_embed_dim // self.num_heads *
                        self.config.rotary_percentage)
            qr = theta_shift(q[..., :n_elem], sin.repeat(1, 2), cos.repeat(1, 2))
            kr = theta_shift(k[..., :n_elem], sin.repeat(1, 2), cos.repeat(1, 2))
            qr = torch.cat([qr, q[..., n_elem:]], dim=-1)
            kr = torch.cat([kr, k[..., n_elem:]], dim=-1)
            # qr = theta_shift(q, sin, cos)
            # kr = theta_shift(k, sin, cos)

        # (B, T, n_h, h_dim) -> (B, n_h, T, h_dim)
        qr = qr.transpose(1, 2)
        kr = kr.transpose(1, 2)
        v = v.transpose(1, 2)
...
class LlamaAttention(nn.Module):
    """This is adapted from the LitGPT implementation, found in TinyLlama repo."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        if apply_rotary_emb_func is not None:
            # (B, T, n_h, h_dim)
            qr = apply_rotary_emb_func(q, cos, sin, False, True)
            kr = apply_rotary_emb_func(k, cos, sin, False, True)
        else:
            n_elem = int(self.config.decoder_embed_dim // self.num_heads *
                        self.config.rotary_percentage)
            qr = theta_shift(q[..., :n_elem], sin.repeat(1, 2), cos.repeat(1, 2), interleave=False)
            kr = theta_shift(k[..., :n_elem], sin.repeat(1, 2), cos.repeat(1, 2), interleave=False)
            qr = torch.cat([qr, q[..., n_elem:]], dim=-1)
            kr = torch.cat([kr, k[..., n_elem:]], dim=-1)
            # qr = theta_shift(q, sin, cos)
            # kr = theta_shift(k, sin, cos)
        attn_output, attn_weights = self.scaled_dot_product_attention(qr, kr, v, T, attention_mask=retention_mask)

        attn_output = attn_output.reshape(B, T, self.embed_dim)
        attn_output = self.out_proj(attn_output)

        if not output_retentions:
            return attn_output, past_key_value
        return attn_output, past_key_value, attn_weights

    def scaled_dot_product_attention(self, q, k, v, seqlen, attention_mask=None):
        if (self.attn_implementation == "flash_attention_2" and
            ...
            ones = torch.ones((seqlen, seqlen), device=q.device, dtype=torch.bool)
            if attention_mask is not None:
                causal_mask = torch.tril(ones).unsqueeze(0) * attention_mask.unsqueeze(1)
            else:
                causal_mask = torch.tril(ones).unsqueeze(0).unsqueeze(0)
            ...
            if self.attn_implementation == "sdpa":
                attn_output = torch.nn.functional.scaled_dot_product_attention(
                    q, k, v, attn_mask=causal_mask, dropout_p=0.0, scale=self.scaling)
                attn_weights = None
            else:
                # no sdpa: eager
                attn_weights = q @ k.transpose(-1, -2) * self.scaling
...
class RetNetDecoderLayer(nn.Module):
    ...
    def __init__(self, config: RetNetConfig, depth: int, tensor_parallel: bool = False):
        ...
        if config.hybrid_attention_layers is not None and depth in config.hybrid_attention_layers:
            # hybrid of attention and retention
            self.retention = LlamaAttention(config, use_bias=config.use_bias)
        else:
...
class RetNetPreTrainedModel(PreTrainedModel):
    ...
    # NOTE: for Hybrid versions that use attention
    _supports_sdpa = True
    ...
    def forward(
        ...
        if retention_rel_pos is None:
            retention_rel_pos = self.retnet_rel_pos(
                ...
                retention_mask=retention_mask,
                get_decay_scale=self.config.retnet_stability_update and use_cache,
            )
